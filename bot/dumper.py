...
def wait_until_day(need_day:int, today: datetime.datetime=datetime.datetime.now(), hour: int = 23, minute: int = 0) -> float:
    ...
    try:
        ...
        with database.DbSchedule() as db:
            ...
            for group in x.take_perpod_list():
                sch = x.schedule(group)
                db.append_json_data(group, json.dumps(sch), False)
